#!/bin/bash
# Comprehensive fix script for SpeedTest app
# This script fixes both database and Nginx issues
echo "=== SpeedTest Comprehensive Fix Script ==="

# Check if running as root
if [ "$EUID" -ne 0 ]; then
  echo "Please run as root (sudo ./comprehensive_fix.sh)"
  exit 1
fi
...
if [ -f "$CONFIG" ]; then
  ...
  echo "✓ Backed up Nginx enabled site"
  ...
  if [ $? -eq 0 ]; then
    ...
    cp "$BACKUP_DIR/$(basename $CONFIG).bak" "$CONFIG"
    echo "✓ Restored Nginx config from backup"
  fi
else
  echo "! Could not find Nginx enabled site configuration"
fi
...
echo "Try accessing http://192.168.8.92 now."